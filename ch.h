...
void mat_trans(cl_mem a, cl_mem b, int N, cl_kernel mat_trans, cl_command_queue queue);

void fft2D(cl_mem a,cl_mem b,cl_mem c,cl_mem d, int N, cl_kernel fft_init, cl_kernel fft1D,cl_kernel mat_trans,cl_command_queue queue,int direction);

//fft first arg devided by q entry-wise and save to second arg
void fft_d_q(cl_mem a,cl_mem b,cl_mem c,cl_mem d, int N,float epsilon,float k ,float s, cl_kernel fft_init,
		cl_kernel fft1D,cl_kernel mat_trans,cl_command_queue queue);

//real(ifft2(fft2(pk)./nlap_s2));
void fft_d_nlaps2(cl_mem a,cl_mem b,cl_mem c,cl_mem d,int N,float epsilon,float k,float s, cl_kernel fft_init,
		cl_kernel fft1D,cl_kernel mat_trans,cl_command_queue queue);

//real(ifft2(fft2(pk).*sharmonic));
void fft_shar(cl_mem a ,cl_mem b ,cl_mem c,cl_mem d,int N,float epsilon,float k,float s, cl_kernel fft_init,
		cl_kernel fft1D,cl_kernel mat_trans,cl_command_queue queue);

// fft(u.^3-u) .*nlap_s
void fft_w_orig(cl_mem a, cl_mem b, cl_mem c,cl_mem d,int N,float epsilon,float k,float s, cl_kernel fft_init,
		cl_kernel fft1D,cl_kernel mat_trans,cl_command_queue queue);


//fft((3*u.^2 -1) .* pk).*nlap_s
void fft_w(cl_mem a, cl_mem b, cl_mem c,cl_mem d,int N,float epsilon,float k,float s, 
...
void chcg(float k,struct parameter * p_param,  cl_mem temp,cl_mem rhs, cl_mem temp2, bool *fail,
		cl_mem temp3,cl_mem temp4,cl_mem temp5,cl_mem temp6,cl_mem temp7,
		cl_mem temp8,cl_mem temp9,cl_kernel fft_init,
